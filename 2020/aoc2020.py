...
def day5():
    ...
    #            (string, F/L, B/R, range min, range max)
    def find_seat(sequence, lower, upper, rmin, rmax):
        seat_pos = 0

        for letter in sequence:
            if   letter == lower:
                rmax -= round((rmax - rmin) / 2)
        ...
        return seat_pos # row / col


    with open("data/day5.txt") as puzzle_input:
        seat_IDs = set()
        max_seat_ID = 0
        ...
        for line in puzzle_input:
            ...
            row = find_seat(rows, 'F', 'B', 0, 127)
            col = find_seat(cols, 'L', 'R', 0, 7)
        ...
        print(f"5.1. The highest seat ID is {max_seat_ID}")
    ...
    print(f"5.2. The ID of my seat is {current-1}")
...
def day6():
    ...
    with open("data/day6.txt") as puzzle_input:
        lines = puzzle_input.readlines()
        for line in lines:
            if line is lines[-1]:
                iterate_line(line)

            # End of group or last input line
            if len(line.strip()) == 0 or line is lines[-1]:
    ...
    print(f"6.1. The total sum of answered questions is {sum_questions}")
    ...
    # Returns 1 if everyone answered, else 0
    def compare(answer):
        for p in group:
            if answer not in p:
                return 0
    ...
    with open("data/day6.txt") as puzzle_input:
        ...
        for line in lines:
            ...
            if len(line.strip()) == 0 or line is lines[-1]:
                for p in group:
                    for a in p:
                        sum_questions += compare(a)
                    break # Avoids duplicate comparisons
                group.clear()

            else:
                group.add(line.strip())

    print(f"6.2. The total sum of answered questions is {sum_questions}")
